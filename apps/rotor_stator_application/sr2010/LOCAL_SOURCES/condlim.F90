...
CONTAINS
  ...
  MODULE SUBROUTINE init_velocity_pressure(mesh_f, mesh_c, time, dt, list_mode, &
    ...
    REAL(KIND=8), DIMENSION(mesh_f%np)         :: x
  ...
  MODULE FUNCTION vv_exact(TYPE,rr,m,t) RESULT(vv)
    ...
    IF (TYPE==5 .AND. m==0) THEN
       DO n = 1, SIZE(rr,2)
          ...
          z= rr(2,n)
          IF( r <  1.063 ) THEN 
                uth = om0*r + om1*r1*r1/r*(1-exp(-r*r/r1/r1))
                uth = uth + om2*r2*r2/r*(1-exp(-r*r/r2/r2))
                uz1 = u0+u1*exp(-r*r/r1/r1)+u2*exp(-r*r/r2/r2)
  ...
  MODULE FUNCTION imposed_velocity_by_penalty(rr,t) RESULT(vv)
    ...
    DO n = 1, SIZE(rr,2)
       ...
       z= rr(2,n)
       IF (z<-0.5d0) THEN
          vv(n,3) =  user%solid_vel(1)*rr(1,n)
       ELSE
          vv(n,3) =  -user%solid_vel(2)*rr(1,n)
       ENDIF
    END DO
    RETURN
  ...
  !===This coefficient is equal to 1 in fluid subdomain
  !===This coefficient is equal to zero in solid subdomain where penalty is applied
  ...
  MODULE FUNCTION penal_in_real_space(mesh,rr_gauss,angles,nb_angles,nb,ne,time) RESULT(vv)
    ...
    !Sharp Blades
    !vv=sharp_penal_in_real_space(mesh,rr_gauss,angles,nb_angles,nb,ne,time)
    RETURN
...
!             CALL RANDOM_SEED
!             CALL RANDOM_NUMBER(x)
!             Hn(:,k,i) = Hn(:,k,i) + ampl*(x - 0.5d0)
!          END IF
!          IF (list_mode(i).GE.2) THEN
!             CALL RANDOM_SEED
!             CALL RANDOM_NUMBER(x)
!             Hn(:,k,i) = ampl*(x - 0.5d0)
!          END IF
...
!!$ ! TM73 smooth
!!$! CN 22/06/2021
!!$    IF ((user%mu_disk - 1.d0).LT.1d-5) THEN
!!$      vv = user%mu_disk
!!$    ELSE
...
!!$    ENDIF
  ...
  MODULE FUNCTION mu_in_real_space(H_mesh,angles,nb_angles,nb,ne,time) RESULT(vv)
    ...
    vv = (1.d0 - penal_in_real_space(H_mesh,H_mesh%rr(:,nb:ne),angles,nb_angles,nb,ne,time))*(user%mu_disk - 1.d0 ) +1.d0

 !!     vv = (1.d0 - smooth_penal_in_real_space(H_mesh,H_mesh%rr(:,nb:ne),angles,nb_angles,nb,ne,time))*(user%mu_disk - 1.d0 ) +1.d0
