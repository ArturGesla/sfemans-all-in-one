...
!
  ...
  CALL initial(vv_mesh, pp_mesh, H_mesh, phi_mesh, temp_mesh, conc_mesh,&
    ...
    WRITE(*,*) 'ds nonlin'
       DO mode=1,m_max_c
         !IF (list_mode(mode).GT.0) THEN
              !un(:,:,mode) = user%amp_nl * un(:,:,mode)
              !pn(:,:,mode) = user%amp_nl * pn(:,:,mode)
              Hn(:,:,mode) = user%amp_nl * Hn(:,:,mode)
              Bn(:,:,mode) = user%amp_nl * Bn(:,:,mode)
         !ENDIF
       ENDDO
    ENDIF
  !===============================================================================
    ...
    IF (once) THEN
       ...
       IF (inputs%type_pb=='nst' .OR. inputs%type_pb=='mhd' .OR. inputs%type_pb=='fhd' &
          ...
          !TEST DCQ
                ...
                !         ENDDO
                ! SYMETRIE Rpi
                CALL val_ener_sym_rpi(comm_one_d_ns(1), vv_mesh, list_mode, un, e_c_u, e_c_u_sym, e_c_u_anti, &
                        type_sym_u, 'u')
       ...
       IF (inputs%type_pb=='mxw' .OR. inputs%type_pb=='mhd' .OR. &
            inputs%type_pb=='mxx' .OR. inputs%type_pb=='fhd' .OR. inputs%type_pb=='mhs') THEN
                ...
                !         ENDDO
                ! SYMETRIE Rpi
                CALL val_ener_sym_rpi(comm_one_d(1), H_mesh, list_mode, Hn, e_cm_h, e_cm_h_sym, e_cm_h_anti, &
                        type_sym_h, 'h')
                ...
                IF (rank_S == 0) THEN
                    ...
                    WRITE(50, "(A)")'#energy per mode in the conductor'
...
!!$       END IF ! end mxw OR mhd OR fhd or mhs
                ...
                IF (rank == 0) THEN
                    WRITE(52, 103)  time, time_average_counter, &
    ...
    IF (MOD(it,inputs%freq_plot) == 0) THEN
                ...
                DO i = 1, m_max_c
                    DO k = 1, 6
                        u_rotated(:, k, i) = time_average_u(:, k, i) &
                                     - vv_exact_transform(k, vv_mesh%rr, list_mode(i), time)
                    END DO
                END DO
                !Plot
                ! TEST HF-CN 140119
       ...
       IF (inputs%type_pb=='mxw' .OR. inputs%type_pb=='mxx' .OR. inputs%type_pb=='mhd' &
          ...
          END IF ! if_plot_2D
                ...
                !!$               vv_mesh, vv_mesh%np,  m_max_c, &
                !!$               mu_field_dummy, mu_field_dummy, nb_procs, bloc_size, m_max_pad,time)
                !!$          !To visualize mu plt
                !!$          CALL vtu_3d(comm_one_d, mu_field_dummy(:,5:6,:), 'H_mesh', 'mu', 'mu', what, opt_it=it_plot)
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!$  END SUBROUTINE FORCES_AND_MOMENTS
    ...
    SUBROUTINE FORCES_AND_MOMENTS(time, vv_mesh, communicator, list_mode, un, torque, torque_top, torque_bot)
        USE def_type_mesh
        USE input_data
        USE boundary
        USE sft_parallele
#include "petsc/finclude/petsc.h"
        ...
        REAL(KIND = 8), DIMENSION(:, :, :), INTENT(IN) :: un
        REAL(KIND = 8), INTENT(IN) :: time
        REAL(KIND = 8), INTENT(OUT) :: torque
        REAL(KIND = 8), INTENT(OUT) :: torque_top
        REAL(KIND = 8), INTENT(OUT) :: torque_bot
        REAL(KIND = 8), DIMENSION(vv_mesh%gauss%l_G * vv_mesh%dom_me, 6, SIZE(list_mode)) :: vel_gauss, vel_gauss_penal
        REAL(KIND = 8), DIMENSION(vv_mesh%gauss%l_G * vv_mesh%dom_me, 6, SIZE(list_mode)) :: vel_gauss_penal_top
        REAL(KIND = 8), DIMENSION(vv_mesh%gauss%l_G * vv_mesh%dom_me, 6, SIZE(list_mode)) :: vel_gauss_penal_bot
        REAL(KIND = 8), DIMENSION(2, vv_mesh%gauss%l_G * vv_mesh%dom_me) :: rr_gauss
        REAL(KIND = 8), DIMENSION(vv_mesh%gauss%l_G * vv_mesh%dom_me) :: flip
        INTEGER, DIMENSION(vv_mesh%gauss%n_w) :: j_loc
        REAL(KIND = 8) :: vel_torque, vel_torque_tot
        REAL(KIND = 8) :: vel_torque_top, vel_torque_top_tot
        REAL(KIND = 8) :: vel_torque_bot, vel_torque_bot_tot
        REAL(KIND = 8) :: torque_loc, torque_top_loc, torque_bot_loc
        ...
        DO m = 1, vv_mesh%dom_me
            ...
            DO l = 1, vv_mesh%gauss%l_G
                ...
                rr_gauss(1, index) = SUM(vv_mesh%rr(1, j_loc) * vv_mesh%gauss%ww(:, l))
                rr_gauss(2, index) = SUM(vv_mesh%rr(2, j_loc) * vv_mesh%gauss%ww(:, l))
                IF (rr_gauss(2, index)>0) THEN
                    flip(index) = -user%solid_vel(2)
                ELSE
                    flip(index) = user%solid_vel(1)
                END IF
            END DO
        END DO
        ...
        DO i = 1, SIZE(list_mode)
            ...
            DO m = 1, vv_mesh%dom_me
                ...
                DO l = 1, vv_mesh%gauss%l_G
                    index = index + 1
                    DO TYPE = 1, 6
                        vel_gauss(index, TYPE, i) = SUM(un(j_loc, TYPE, i) * vv_mesh%gauss%ww(:, l))
                    END DO
                END DO
            END DO
            IF(inputs%if_impose_vel_in_solids) THEN
            ...
            END IF
        END DO
        vel_gauss = vel_gauss * (3 / (2 * inputs%dt))
        ...
        CALL FFT_PAR_VAR_ETA_PROD_GAUSS_DCL(communicator(2), penal_in_real_space_top, vv_mesh, &
                vel_gauss, vel_gauss_penal_top, nb_procs, bloc_size, m_max_pad, rr_gauss, time)

        CALL FFT_PAR_VAR_ETA_PROD_GAUSS_DCL(communicator(2), penal_in_real_space_bot, vv_mesh, &
                vel_gauss, vel_gauss_penal_bot, nb_procs, bloc_size, m_max_pad, rr_gauss, time)
        ...
        DO i = 1, SIZE(list_mode)
            ...
            ELSE
                ...
                DO m = 1, vv_mesh%dom_me
                    ...
                    DO l = 1, vv_mesh%gauss%l_G
                        index = index + 1
                        !===Torque is int_domain ((1-chi)*(u-u_solid)/dt x X )dx; chi=1 in fluid, 0 in solid
                        vel_torque = vel_torque + flip(index) * rr_gauss(1, index) * &
                                (vel_gauss_penal(index, 3, i) - vel_gauss(index, 3, i)) &
                                * rr_gauss(1, index) * vv_mesh%gauss%rj(l, m)
                        vel_torque_top = vel_torque_top + flip(index) * rr_gauss(1, index) * &
                                (vel_gauss_penal_top(index, 3, i) - vel_gauss(index, 3, i)) &
                                * rr_gauss(1, index) * vv_mesh%gauss%rj(l, m)
                        vel_torque_bot = vel_torque_bot + flip(index) * rr_gauss(1, index) * &
                                (vel_gauss_penal_bot(index, 3, i) - vel_gauss(index, 3, i)) &
                                * rr_gauss(1, index) * vv_mesh%gauss%rj(l, m)
                    END DO
                END DO
                CALL MPI_ALLREDUCE(vel_torque, vel_torque_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(1), ierr)
                CALL MPI_ALLREDUCE(vel_torque_top, vel_torque_top_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(1), ierr)
                CALL MPI_ALLREDUCE(vel_torque_bot, vel_torque_bot_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(1), ierr)
                WRITE(*, *) ' FORCES_AND_MOMENTS ', time, 2 * ACOS(-1.d0) * vel_torque_tot / 2
                WRITE(666, *) time, 2 * ACOS(-1.d0) * vel_torque_tot / 2
                torque_loc = 2 * ACOS(-1.d0) * vel_torque_tot / 2
                torque_top_loc = 2 * ACOS(-1.d0) * vel_torque_top_tot / 2
                torque_bot_loc = 2 * ACOS(-1.d0) * vel_torque_bot_tot / 2
            END IF
        END DO
        CALL MPI_ALLREDUCE(torque_loc, torque, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(2), ierr)
        CALL MPI_ALLREDUCE(torque_top_loc, torque_top, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(2), ierr)
        CALL MPI_ALLREDUCE(torque_bot_loc, torque_bot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, communicator(2), ierr)
! CN 02/2024
        WRITE(665, 103) time, torque, torque_top, torque_bot
...
#include "petsc/finclude/petsc.h"
    ...
    IF (my_petscworld_rank==0) THEN
       ...
       WRITE(197,103)  time, ABS(1-inte_fft_tot/(volum_init+1.d-14))
    END IF
        ...
        IF (inputs%type_pb=='nst' .OR. inputs%type_pb=='mhd') THEN
            ...
            IF (rank == 0) THEN
                ...
                WRITE(97, 110)    time, time_average_counter, energy, delta, &
                        urms, urms / (user%disk_radius * user%solid_vel(:))
            END IF
        END IF !nst or mhd
...
! CN 02/2024
    ...
    !TEST LC
    FUNCTION penal_in_real_space_top(mesh, rr_gauss, angles, nb_angles, nb, ne, time) RESULT(vv)
        IMPLICIT NONE
        TYPE(mesh_type) :: mesh
        ...
        END DO
        RETURN
    END FUNCTION penal_in_real_space_top

    FUNCTION penal_in_real_space_bot(mesh, rr_gauss, angles, nb_angles, nb, ne, time) RESULT(vv)
        IMPLICIT NONE
        TYPE(mesh_type) :: mesh
        REAL(KIND = 8), DIMENSION(:, :), INTENT(IN) :: rr_gauss
        REAL(KIND = 8), DIMENSION(:), INTENT(IN) :: angles
        ...
        END DO
        RETURN
    END FUNCTION penal_in_real_space_bot
    ...
    FUNCTION top_propeller(r, z, angles, nb_angles, time) RESULT(vv)
        ...
        REAL(KIND = 8) :: top_propeller_angle_offset = 0.d0
        INTEGER :: nblades = 8 !number of blades
        REAL(KIND = 8) :: lw = 0.0125 !lw= is the half thickness of the blades
        REAL(KIND = 8) :: disk_bot = -1.02d0, top_of_disk_bot = -0.9d0, top_of_blade_bot = -0.7d0
        REAL(KIND = 8) :: disk_top = 1.02d0, bot_of_disk_top = 0.9d0, bot_of_blade_top = 0.7d0
        REAL(KIND = 8) :: hole_radius = 0.1d0
!!!        REAL(KIND = 8) :: disk_radius = 0.925d0, hole_radius = 0.1d0
        REAL(KIND = 8) :: two_rp != disk_radius/SIN(twopi*72.d0/360.d0)
!!!        REAL(KIND = 8) :: user%omega_Vol!=pi*2.48 !pi*r^2*h

        twopi = 2 * pi
        two_rp = user%disk_radius / SIN(twopi * 72.d0 / 360.d0)
!!!        omega_Vol = pi * 2.48
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !===TM73
...
!        REAL(KIND = 8) :: two_rp != disk_radius/SIN(twopi*24.d0/360.d0)
!!!!        REAL(KIND = 8) :: omega_Vol!=pi*(1.0)*(2.0) !pi*r^2*h !Volume of the cylinder (all domain)
!
!        twopi = 2 * pi
!        two_rp = user%disk_radius / SIN(twopi*24.d0/360.d0)
!!!!        omega_Vol = pi * 2.d0
...
!!!        REAL(KIND = 8) :: disk_radius = 0.925d0, hole_radius = 0.1d0
        REAL(KIND = 8) :: two_rp != disk_radius/SIN(twopi*72.d0/360.d0)
!!!        REAL(KIND = 8) :: omega_Vol!=pi*2.48 !pi*r^2*h
