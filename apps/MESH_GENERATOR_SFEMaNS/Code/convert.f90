...
CONTAINS
   ...
   SUBROUTINE g_load (idf, ascii, grid)
      ...
      INTEGER, INTENT(IN) :: idf
      LOGICAL, INTENT(IN) :: ascii
!EN TEST      TYPE (domain), INTENT(INOUT) :: grid
      TYPE (domain), INTENT(INOUT) :: grid
   ...
   SUBROUTINE g_copy (g_old, g_new)
      ...
      TYPE (domain), INTENT(INOUT) :: g_old
      ...
      DO e=1,ne
	 m = np_b
	 CALL set_p2i (np_b,   g_old % edges(e) % pnt_head)
	 CALL p_list  (np_b-m, g_new % edges(e) % pnt_head)
	 m = np_b
   ...
   SUBROUTINE p_save (idf, ascii, hp)
      ...
      INTEGER, INTENT(IN) :: idf
      LOGICAL, INTENT(IN) :: ascii
      TYPE (point), TARGET, INTENT(INOUT) :: hp
      ...
      IF ( ascii ) THEN
	    ...
	    ! WRITE (idf,*)
	    ! WRITE (idf,'(8x,1l)')  p % visited
	    ! WRITE (idf,'(3i10)')   old, ppp, spx
	    ! WRITE (idf,'(3i10)')   p % cnd, p % ibk, p % parent
	    ! WRITE (idf,'(3e15.7)') p % leng
	    ! WRITE (idf,'(3e15.7)') p % x
	    ! WRITE (idf,'(3e15.7)') p % metric
   ...
   SUBROUTINE s_save (idf, ascii, nd, hs)
      ...
      LOGICAL, INTENT(IN) :: ascii
      TYPE (simplex), TARGET, INTENT(INOUT) :: hs
      ...
      IF ( ascii ) THEN
	    ...
	    ! WRITE (idf,*)
	    ! WRITE (idf,'(2i10)')   int, ext
	    ! WRITE (idf,'(6i10)')  (s2p(i),i=1,nd+1), (s2s(i),i=1,nd+1)
	    ! WRITE (idf,'(i10)')    s % status
	    ! WRITE (idf,'(4i10)')   s % cmat
	    ! WRITE (idf,'(4e15.7)') s % det
	    ! WRITE (idf,'(4e15.7)') s % fmat
   ...
   SUBROUTINE p_load (idf, ascii, m, n, i2p, i2s, i2ppp)
      ...
      LOGICAL, INTENT(IN) :: ascii
      TYPE (p_pnter), DIMENSION(:), INTENT(INOUT) :: i2p, i2ppp
   ...
   SUBROUTINE s_load (idf, ascii, nd, m, n, i2p, i2s, i2sss)
      ...
      TYPE (s_pnter), DIMENSION(:), INTENT(INOUT) :: i2s, i2sss
   ...
   SUBROUTINE p_copy (hp_old, i2p, i2s, i2ppp)
      ...
      TYPE (point), TARGET, INTENT(INOUT) :: hp_old
      TYPE (p_pnter), DIMENSION(:), INTENT(INOUT) :: i2p, i2ppp
      TYPE (s_pnter), DIMENSION(:), INTENT(INOUT) :: i2s
   ...
   SUBROUTINE s_copy (hs_old, nd, i2p, i2s, i2sss)
      ...
      TYPE (simplex), TARGET, INTENT(INOUT) :: hs_old
   ...
   SUBROUTINE p_list (n, p_head)
      ...
      INTEGER, INTENT(IN) :: n
      TYPE (point), TARGET, INTENT(INOUT) :: p_head
   ...
   SUBROUTINE s_list (n, s_head)
      ...
      INTEGER, INTENT(IN) :: n
      TYPE (simplex), TARGET, INTENT(INOUT) :: s_head
   ...
   SUBROUTINE set_i2s_b (n, head, i2s)
      ...
      INTEGER, INTENT(INOUT) :: n
      TYPE (simplex), TARGET, INTENT(INOUT) :: head
      TYPE (s_pnter), DIMENSION(:), INTENT(OUT) :: i2s
