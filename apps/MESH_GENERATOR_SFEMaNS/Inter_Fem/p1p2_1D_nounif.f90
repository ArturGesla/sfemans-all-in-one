PROGRAM p1p2_1d
  ...
  INTEGER :: isopar
  REAL(KIND=8) :: xmin, xmax, ymin, ymax, h, harvest
  LOGICAL :: form
  !-------------INITIAL MESH OPERATIONS
  OPEN(21,FILE='data_und',FORM='formatted', STATUS='unknown')
  READ(21,*) form
  write(*,*) 'Form', form
  READ(21,*) xmin, xmax, ymin, ymax, h
  directory = '.'
  READ(21,*)  file_name
  CLOSE(21)
  ...
  DO n = 1, np_c_1d
     IF (n>1 .AND. n<np_c_1d) THEN
        CALL RANDOM_NUMBER(harvest)
     ELSE
        harvest = 0.5d0
     END IF
     rr_c(1,2*(n-1)+1) = xmin + (n-1)*h + (harvest-0.5d0)*h/3
     rr_c(1,2*(n-1)+2) = xmin + (n-1)*h + (harvest-0.5d0)*h/3
     rr_c(2,2*(n-1)+1) = ymin
     rr_c(2,2*(n-1)+2) = ymax
  END DO
  ...
  CALL plot_pressure_p1_label(jj_c, rr_c, rr_c(1,:), 'maill_G_grossier.plt')
  ...
  IF (form) THEN
     ...
     DO m = 1, SIZE(jj_c_fin, 2)
        WRITE(10, *)  (jj_c_fin(n,m), n = 1,nw_c),  (neigh_fin(n,m), n = 1,nw_c), id(m)
     END DO
     ...
     DO m = 1, me_fin
        WRITE(10,*) jj(:,m), neigh_fin(:,m), id(m)
     END DO

     DO ms = 1, mes_fin
        WRITE(10,*) jjs(:,ms),neighs_fin(ms), sides_fin(ms)
     END DO
  ...
  ELSE
     ...
     WRITE(10) jjs, neighs_fin, sides_fin
     WRITE(10) rr
  END IF
