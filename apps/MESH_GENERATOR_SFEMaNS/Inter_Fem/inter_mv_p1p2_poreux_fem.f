...
!POREUX
   REAL(KIND=8) :: epsilon=1.d-9, r0, dist
...
!POREUX
            ...
            STOP
100         IF (bord.EQ.2) THEN 
               centre = (/0.d0,1.d0/)
               index = 1
            ELSE IF (bord.EQ.4) THEN 
               centre = (/1.d0,1.d0/)
               index = 1
            ELSE IF (bord.EQ.6) THEN 
               centre = (/1.d0,0.d0/)
               index = 1
            ELSE IF (bord.EQ.8) THEN 
               centre = (/0.d0,0.d0/)
               index = 1
            ELSE IF (bord.EQ.9) THEN 
               centre = (/0.5d0,0.5d0/)
               index = 1
            ELSE 
               index = 0
            ...
            IF (index.EQ.1) THEN
               r_mid = r_mid - centre
               dist = SQRT(SUM((r_mid)**2))  
               r_mid = centre + r0*r_mid/dist
               rr_f(:, n) = r_mid
            END IF
