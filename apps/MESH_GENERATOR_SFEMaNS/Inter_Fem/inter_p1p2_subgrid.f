...
USE sub_plot
   ...
   INTEGER, ALLOCATABLE, DIMENSION(:)        :: neighs_fin, sides_fin 
...
!-------------INITIAL MESH OPERATIONS 
   CALL edit(file_name,i_d,index_edge)
   CALL geometry(file_name)
...
!      endif
   ...
   WRITE(*,*) ' Maillage isoparametrique (1) ou subparametrique (0)?'
   READ(*,*) isopar 
...
!--------Creation du maillage P1/P2 fin
   ...
   ALLOCATE(jj_c_fin(nw_c,me_fin),    neigh_fin(nw_c,me_fin), &
            jjs_c_fin(nws_c,mes_fin), neighs_fin(mes_fin),    sides_fin(mes_fin),& 
            rr_c_fin(kd, np))
...
!   DEALLOCATE(jj,jjs,rr)
   ...
   write(10,'(1x,10(i7,1x))') np_c_fin, nw_c, me_fin, nws_c, mes_fin

   DO m = 1, SIZE(jj_c_fin, 2)
      WRITE(10, '(1x,10(i7,1x))')  (jj_c_fin(n,m), n = 1,nw_c),  (neigh_fin(n,m), n = 1,nw_c),  i_d
   END DO

   DO ms = 1, SIZE(jjs_c_fin, 2)
      WRITE(10, '(1x,10(i7,1x))')  (jjs_c_fin(n,ms), n = 1,nws_c),  neighs_fin(ms), sides_fin(ms)
   END DO
   ...
   WRITE  (10, '(1x,10(i7,1x))') np, nw, me_fin, nws, mes_fin

   DO m = 1, me_fin
      WRITE(10,'(1x,10(i7,1x))') jj(:,m),  (neigh_fin(n,m), n = 1,nw_c),  i_d
   END DO

   DO ms = 1, mes_fin
      WRITE(10,'(1x,10(i7,1x))') jjs(:,ms), neighs_fin(ms), sides_fin(ms)
   END DO
   ...
   WRITE (*,*)  'np_c',np_c,'nwc',nw_c,'nwsc',nws_c,'me',me,'mes',mes
   WRITE (*,*)  'np',np,'nw',nw,'nws ',nws
   WRITE(*,*) ' Ecriture de maill_fin.plt'
   CALL plot_pressure_p1_label(jj_c_fin, rr_c_fin, rr_c_fin(1,:), &
                               'maill_grossier.plt')
